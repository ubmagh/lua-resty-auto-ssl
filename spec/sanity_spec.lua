...
describe("sanity", function()
  ...
  it("returns the fallback SSL certificate when SNI isn't used", function()
    ...
    local error_log = server.read_error_log()
    assert.matches("could not determine domain for request (SNI not supported?) - using fallback - ", error_log, nil, true)
    assert.Not.matches("[error]", error_log, nil, true)
    assert.Not.matches("[alert]", error_log, nil, true)
    assert.Not.matches("[emerg]", error_log, nil, true)
  end)
  ...
  it("disallows all domains for registration by default", function()
    ...
    local error_log = server.read_error_log()
    assert.matches("domain not allowed - using fallback - " .. server.tunnel_hostname, error_log, nil, true)
    assert.Not.matches("[error]", error_log, nil, true)
    assert.Not.matches("[alert]", error_log, nil, true)
    assert.Not.matches("[emerg]", error_log, nil, true)
  end)
  ...
  it("returns the fallback SSL certificate when the domain is allowed and valid, but the domain challenge fails", function()
    ...
    local error_log = server.read_error_log()
    assert.matches("issuing new certificate for not-ours-" .. server.tunnel_hostname, error_log, nil, true)
    assert.matches("dehydrated failed", error_log, nil, true)
    assert.matches("could not get certificate for not-ours-" .. server.tunnel_hostname, error_log, nil, true)
    assert.Not.matches("[alert]", error_log, nil, true)
    assert.Not.matches("[emerg]", error_log, nil, true)
  end)
  ...
  it("returns the fallback SSL certificate when the domain is allowed, but not resolvable", function()
    ...
    local error_log = server.read_error_log()
    assert.matches("issuing new certificate for unresolvable-sdjfklsdjf.example", error_log, nil, true)
    assert.matches("dehydrated failed", error_log, nil, true)
    assert.matches("could not get certificate for unresolvable-sdjfklsdjf.example", error_log, nil, true)
    assert.Not.matches("[alert]", error_log, nil, true)
    assert.Not.matches("[emerg]", error_log, nil, true)
  end)
  ...
  it("allows for custom logic to control domain name to handle lack of SNI support", function()
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.Not.matches("could not determine domain for request (SNI not supported?) - using fallback - ,", error_log, nil, true)
      assert.matches("issuing new certificate for non-sni-9444.example", error_log, nil, true)
      assert.matches("dehydrated failed", error_log, nil, true)
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.Not.matches("could not determine domain for request (SNI not supported?) - using fallback - ,", error_log, nil, true)
      assert.matches("issuing new certificate for non-sni-mismatch-9445.example", error_log, nil, true)
      assert.matches("dehydrated failed", error_log, nil, true)
      assert.matches("Domain name does not end with a valid public suffix (TLD)", error_log, nil, true)
      assert.Not.matches("[alert]", error_log, nil, true)
      assert.Not.matches("[emerg]", error_log, nil, true)
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.matches("could not determine domain for request (SNI not supported?) - using fallback - ,", error_log, nil, true)
      assert.Not.matches("issuing new certificate", error_log, nil, true)
      assert.Not.matches("[error]", error_log, nil, true)
      assert.Not.matches("[alert]", error_log, nil, true)
      assert.Not.matches("[emerg]", error_log, nil, true)
    end
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.Not.matches("could not determine domain for request (SNI not supported?) - using fallback - ,", error_log, nil, true)
      assert.matches("domain not allowed - using fallback - non-sni-disallowed-9447.example", error_log, nil, true)
      assert.Not.matches("issuing new certificate", error_log, nil, true)
      assert.Not.matches("[warn]", error_log, nil, true)
      assert.Not.matches("[error]", error_log, nil, true)
      assert.Not.matches("[alert]", error_log, nil, true)
  ...
  it("deletes dehydrated temporary files if cert deployment fails", function()
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.matches("issuing new certificate for", error_log, nil, true)
      assert.matches("failed to open file for writing:", error_log, nil, true)
      assert.matches("failed to set cert", error_log, nil, true)
      assert.matches("dehydrated failed", error_log, nil, true)
      assert.matches("issuing new certificate failed", error_log, nil, true)
      assert.matches("could not get certificate for", error_log, nil, true)
    end

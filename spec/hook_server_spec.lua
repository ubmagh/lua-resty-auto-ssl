...
describe("hook server", function()
  ...
  it("without secret", function()
    ...
    assert.matches("unauthorized access to hook server (hook secret did not match)", error_log, nil, true)
  ...
  it("unknown path", function()
    ...
    local error_log = server.read_error_log()
    assert.matches("unknown request to hook server: /foobar", error_log, nil, true)
    assert.Not.matches("[warn]", error_log, nil, true)
    assert.Not.matches("[alert]", error_log, nil, true)
    assert.Not.matches("[emerg]", error_log, nil, true)
  end)
  ...
  it("POST body execeeds buffer size", function()
    ...
    if ngx.config.ngx_lua_version < 10008 then -- v0.10.8
      assert.matches("failed to parse POST args: requesty body in temp file not supported", error_log, nil, true)
    else
      assert.matches("failed to parse POST args: request body in temp file not supported", error_log, nil, true)
    end
    assert.Not.matches("[alert]", error_log, nil, true)
    assert.Not.matches("[emerg]", error_log, nil, true)
  end)

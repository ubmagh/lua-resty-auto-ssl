...
describe("renewal", function()
  ...
  it("calls the allow_domain callback on renewals", function()
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.Not.matches("issuing new certificate for " .. server.tunnel_hostname, error_log, nil, true)
    ...
    do
      ...
      assert.Not.matches("issuing new certificate for " .. server.tunnel_hostname, error_log, nil, true)
      assert.matches("checking certificate renewals for " .. server.tunnel_hostname, error_log, nil, true)
      assert.Not.matches("expiry date is more than configured `renew_age_days` days out, skipping renewal: " .. server.tunnel_hostname, error_log, nil, true)

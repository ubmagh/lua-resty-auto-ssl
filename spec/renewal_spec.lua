...
describe("renewal", function()
  ...
  it("calls the allow_domain callback on renewals", function()
    ...
    do
      ...
      local error_log = server.nginx_error_log_tail:read()
      assert.Not.matches("auto-ssl: issuing new certificate for " .. server.tunnel_hostname, error_log, nil, true)
    ...
    do
      ...
      assert.Not.matches("auto-ssl: issuing new certificate for " .. server.tunnel_hostname, error_log, nil, true)
      assert.matches("auto-ssl: checking certificate renewals for " .. server.tunnel_hostname, error_log, nil, true)
      assert.Not.matches("auto-ssl: expiry date is more than 30 days out, skipping renewal: " .. server.tunnel_hostname, error_log, nil, true)
      ...
      assert.matches("auto-ssl: domain not allowed, not renewing: " .. server.tunnel_hostname, error_log, nil, true)
    end
  end)
end)

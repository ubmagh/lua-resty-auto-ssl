...
return function(auto_ssl_instance)
  ...
  if mkdir_challenges_err then
    ngx.log(ngx.ERR, "[auto-ssl][init_worker]: failed to create letsencrypt/.acme-challenges dir: ", mkdir_challenges_err)
  end
  ...
  if mkdir_locks_err then
    ngx.log(ngx.ERR, "[auto-ssl][init_worker]: failed to create letsencrypt/locks dir: ", mkdir_locks_err)
  end

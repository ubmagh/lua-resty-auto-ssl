...
local function start()
  ...
  if set_false_err then
    ngx.log(ngx.ERR, "[auto-ssl][start_sockproc]: failed to set shdict for sockproc_started: ", set_false_err)
  end

  ngx.log(ngx.NOTICE, "[auto-ssl][start_sockproc]: starting sockproc")

  local _, run_err = shell_blocking.capture_combined({ auto_ssl.lua_root .. "/bin/resty-auto-ssl/start_sockproc" }, { umask = "0022" })
  if run_err then
    ngx.log(ngx.ERR, "[auto-ssl][start_sockproc]: failed to start sockproc: ", run_err)
  else
    local _, set_err = ngx.shared.auto_ssl_settings:safe_set("sockproc_started", true)
    if set_err then
      ngx.log(ngx.ERR, "[auto-ssl][start_sockproc]: failed to set shdict for sockproc_started: ", set_err)
    end
  end
end
...
return function(force)
  ...
  if new_lock_err then
    ngx.log(ngx.ERR, "[auto-ssl][start_sockproc]: Failed to create lock: ", new_lock_err)
    return
  end
  ...
  if not ok then
    ngx.log(ngx.ERR, "[auto-ssl][start_sockproc]: failed to run start_sockproc: ", err)
  end

  local unlock_ok, unlock_err = start_lock:unlock()
  if not unlock_ok then
    ngx.log(ngx.ERR, "[auto-ssl][start_sockproc]: failed to unlock: ", unlock_err)
  end
end

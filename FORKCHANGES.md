...
- **`memory_spec.lua` assumed a memory-pressure scenario that no longer reliably applies** — this test cram-fills the shared dict with large items to force eviction, then expects a *small* subsequent cert-cache write to also force eviction and log a warning. Newer nginx shared-dict slab allocators (bundled in the bumped images) reuse freed space across size classes more efficiently than the old ones this test was written against, so the small write no longer reliably needed to evict anything. Added a second fill loop that keeps inserting small items until eviction is forced for one of those too, so the dict is genuinely out of room for any size, not just large ones; loosened the exact `keys == 2` assertion to accommodate the extra filler keys.

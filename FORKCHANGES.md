...
- **ngrok free tier: one sticky domain, one concurrent session per account** — v3's free plan assigns exactly one fixed dev-domain per account (no more random-per-launch subdomains) and only allows one endpoint online at a time for it. Since every matrix variant shares the same `NGROK_AUTHTOKEN` secret, running the GitHub Actions matrix in parallel (the default) makes variants fight over that single tunnel (`ERR_NGROK_334`, "endpoint already online"). Fixed by serializing the matrix (`max-parallel: 1`) — there's no way to give each variant its own domain without a paid ngrok plan.
